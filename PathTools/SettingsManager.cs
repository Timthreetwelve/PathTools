...
namespace TKUtils
{
    ...
    public abstract class SettingsManager<T> where T : SettingsManager<T>, new()
    {
        ...
        private static void GetSettingsFile(string Folder, string FileName)
        {
            ...
            {
                "LOCAL_APPDATA_FOLDER" => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                                                       companyName,
                                                       appName),
                "APPDATA_FOLDER" => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                                                 companyName,
                                                 appName),
                "APPLICATION_FOLDER" => Path.GetDirectoryName(Assembly.GetEntryAssembly().Location),
                _ => Folder,
            };
